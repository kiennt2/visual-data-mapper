...
const helper: any = {
    ...
    hasCssClass(event: any, name: string): boolean {
        if (event && event.target) {
            const classes: any = event.target.className?.split(' ')
            return (classes && classes.length && classes.includes(name)) || (event.target.getAttribute('data-css-class') === name)
        }
        return false
    },
    getButtonId(event: any): string {
        if (event && event.target) {
            return event.target.getAttribute('data-id') || event.target.id
        }
        return ''
    },
    shortenData(obj: any): any {
        let data: any = lodash.cloneDeep(obj)
    ...
    compareValueThenGetDiffProps(obj1: any, obj2: any): string[] | number[] {
        // @ts-ignore
        return lodash.reduce(obj1, (result: string[] | number[], value, key: string) => lodash.isEqual(value, obj2[key]) ? result : result.concat(key), [])
    },
    ...
    getDefaultValueByKind(kind: string): string {
        let value: any
        switch (kind) {
            case 'null':
                value = null
