...
exports.JsonMapping = void 0;
const layout_1 = __importStar(require("./layout"));
const types_1 = require("./types");
const events_1 = require("events");
const map_factory_1 = __importStar(require("map-factory"));
const helper_1 = __importDefault(require("./helper"));
const kind_of_1 = __importDefault(require("kind-of"));
const lodash_1 = __importDefault(require("lodash"));
//
const JSONTreeView = require('json-tree-view');
class JsonMapping extends events_1.EventEmitter {
    constructor(opt) {
        super();
        this.options = lodash_1.default.cloneDeep(types_1.defaultOptions);
        this.domIds = {
            container: '',
            table: '',
            tableBody: '',
        ...
        this.init = (container) => {
            ...
            if (container) {
                ...
                const layout = (0, layout_1.default)(i18nObj);
                const popup = (0, layout_1.getPopupLayout)(i18nObj);
        ...
        this.setConfigs = (configs) => {
            ...
            if (conf) {
                const lastOptions = lodash_1.default.cloneDeep(this.options);
                const obj = {};
                Object.keys(this.options).forEach((k) => {
                    if (lodash_1.default.has(conf, k)) {
                        if (k === 'text') {
                            // make sure i18n keys exists
                            obj.text = lodash_1.default.merge({}, lodash_1.default.cloneDeep(types_1.defaultOptions.text), conf.text);
                        }
                        else {
                            obj[k] = conf[k];
                        }
                ...
                this.options = lodash_1.default.merge({}, this.options, obj);
                this.doUpdate(lastOptions);
                this.emit('configs', lastOptions, lodash_1.default.cloneDeep(this.options));
            }
            else {
        ...
        this.doUpdate = (lastOptions) => {
            var _a;
            const lastTargetSchema = lodash_1.default.cloneDeep(this.targetSchema);
            const lastSourceSchema = lodash_1.default.cloneDeep(this.sourceSchema);
            this.targetSchema = this.getSchema(this.options.target, this.options.shortenTarget);
            this.sourceSchema = this.getSchema(this.options.source, this.options.shortenSource);
            const targetEqual = lodash_1.default.isEqual(lastTargetSchema, this.targetSchema);
            const sourceEqual = lodash_1.default.isEqual(lastSourceSchema, this.sourceSchema);
            ...
            this.i18nUpdate(helper_1.default.compareValueThenGetDiffProps(lastOptions.text, this.options.text));
        ...
        this.getSchema = (data, shorten) => {
            return shorten ? helper_1.default.shortenData(data) : lodash_1.default.cloneDeep(data) || {};
        };
        this.importTarget = () => {
            let obj = undefined;
            ...
            if (txt) {
                ...
                if (!obj || (obj === '')) {
                    ...
                    this.setTargetTreeValue(lodash_1.default.cloneDeep(this.targetSchema));
        ...
        this.importSource = () => {
            ...
            if (txt) {
                ...
                if (!obj || (obj === '')) {
                    ...
                    this.setSourceTreeValue(lodash_1.default.cloneDeep(this.sourceSchema));
        ...
        this.setRowsHtmlToTbody = () => {
            ...
            if (tbody && this.fields.length) {
                let html = '';
                this.fields.map((f) => {
                    html += (0, layout_1.RowHtml)(f, this.options.indent, { text: this.options.text });
                });
        ...
        this.buildTableRows = (data) => {
            ...
            this.fields = (0, layout_1.GetMappingFields)(data);
            this.setRowsHtmlToTbody();
        };
        this.updateRows = () => {
            this.removeRowsListener();
        ...
        this.setDisabledToMultipleField = () => {
            ...
            this.fields = this.fields.map((f) => {
                ...
                lastFieldHasSourcePath = (0, layout_1.hasSourcePath)(f.sourceFields) && !f.disabled;
        ...
        this.handleRowListener = (row, remove) => {
            if (row) {
                ...
                const inputs = row.getElementsByClassName(types_1.defaultInputCssClass);
        ...
        this.onRowClick = (e) => {
            const currentTarget = e.target.getAttribute(types_1.targetPathAttr);
            if (helper_1.default.hasCssClass(e, types_1.RowButtons.SelectSourcePath)) {
                this.selectSourcePath(currentTarget, parseInt(e.target.getAttribute('data-index')));
            }
            else if (helper_1.default.hasCssClass(e, types_1.RowButtons.AddMoreSourceField)) {
                this.addMoreSourceField(currentTarget);
            }
            else if (helper_1.default.hasCssClass(e, types_1.RowButtons.RemoveSourceField)) {
                this.removeSourceField(currentTarget, parseInt(e.target.getAttribute('data-index')));
            }
            else if (helper_1.default.hasCssClass(e, types_1.RowButtons.ClearSourcePathData)) {
        ...
        this.rowOnInputChange = (e) => {
            const currentTarget = e.target.getAttribute(types_1.targetPathAttr);
            if (helper_1.default.hasCssClass(e, types_1.RowInputs.InputDefaultValue)) {
                this.onInputDefaultValueChange(currentTarget, e.target.value);
            }
            else if (helper_1.default.hasCssClass(e, types_1.RowInputs.InputTransformData)) {
            ...
            if (targetPath) {
                this.fields = this.fields.map((f) => {
                    if (f.targetPath === targetPath) {
                        ...
                        (0, layout_1.redrawRow)(this.domIds.tableBody, targetPath, f, this.options.indent, { text: this.options.text }, this.removeListenerFromRow, this.handleRowListener);
                    }
                    return f;
                });
                this.emitChange();
                    ...
                    if (f.targetPath === targetPath) {
                        f.sourceFields = f.sourceFields.map((s, idx) => {
                            ...
                            if (index === idx) {
                                ...
                                s.sourceDataType = path.realPath && path.realPath.length ? (0, kind_of_1.default)(lodash_1.default.get(this.sourceSchema, path.realPath)) : '';
                        ...
                        if (this.options.disableChildWhenParentHasSource && f.disabled) {
                            (0, layout_1.redrawRow)(this.domIds.tableBody, f.targetPath, f, this.options.indent, { text: this.options.text }, this.removeListenerFromRow);
                        }
                    }
                    return f;
                });
                if (!currentRowHasSourceData && this.options.disableChildWhenParentHasSource) {
                    this.fields = this.fields.map((f) => {
                        if (f.targetPath.indexOf(targetPath) === 0) {
                            f.disabled = false;
                            (0, layout_1.redrawRow)(this.domIds.tableBody, f.targetPath, f, this.options.indent, { text: this.options.text }, this.removeListenerFromRow, this.handleRowListener);
                        }
                        return f;
                    });
                }
        ...
        this.getDefaultTransform = (source) => {
            if (source.length === 1) {
                return types_1.defaultTransform;
            }
            let fn = '';
            let result = '';
            source.map((s, index) => {
        ...
        this.handleSingleButtonClick = (e) => {
            const id = helper_1.default.getButtonId(e);
            switch (id) {
        ...
        this.handleCopyEvent = (e) => {
            try {
                ...
                if (ele) {
                    helper_1.default.copy(ele.value);
                }
            }
            catch (e) {
                console.log(e);
        ...
        this.execute = (resultAsString) => {
            const mapper = (0, map_factory_1.default)(types_1.MapperOptions);
            const getValue = map_factory_1.getValue;
            const setValue = map_factory_1.setValue;
            const source = this.sourceSchema;
            let output = '';
            try {
                let fields = (0, layout_1.mapperFields)(this.fields);
                if (fields) {
                    output = eval('(mapper' + fields + ')');
                    const fieldsWithSourcePathEmpty = (0, layout_1.mapperGroupFieldsBySourcePath)(this.fields, 'emptyPath');
        ...
        this.setDataToCodeArea = (returnData) => {
            const code = (0, layout_1.jsCode)(this.fields, this.sourceSchema);
        ...
        this.setDataToOptionsArea = (fromDefault) => {
            const area = document.getElementById(this.domIds.optionsArea);
            if (area) {
                const opt = fromDefault ? lodash_1.default.cloneDeep(types_1.defaultOptions) : lodash_1.default.cloneDeep(this.options);
                // @ts-ignore
